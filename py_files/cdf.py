...
def cdf_and_q(pdf, xs):
    ...
    F = interp1d(xs, cdf_vals, bounds_error=False, fill_value=(0.0, 1.0))
    ...
    #here we are making the inverse of the CDF, so that we can map from the uniform distribution to the original pdf

    # generate samples
    u_samples = np.random.rand(1000000)    # uniform(0,1)
    x_samples = Q(u_samples)            # map to your PDF

    # apply CDF transform back to uniform
    u_check = F(x_samples)              # should be ~ uniform
